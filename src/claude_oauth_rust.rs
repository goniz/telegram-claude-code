...
//! # Examples
//!
//! `no_run //! use claude_oauth::{ClaudeAuth, Config}; //!  //! #[tokio::main] //! async fn main() -> Result<(), Box<dyn std::error::Error>> { //!     let config = Config::default(); //!     let auth = ClaudeAuth::new(config); //!      //!     // Generate login URL //!     let url = auth.generate_login_url().await?; //!     println!("Visit: {}", url); //!      //!     // Exchange authorization code for tokens //!     let auth_code = "your_auth_code_here"; //!     let credentials = auth.exchange_code(auth_code).await?; //!      //!     // Save credentials //!     auth.save_credentials(&credentials).await?; //!      //!     Ok(()) //! } //! `
