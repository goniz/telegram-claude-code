...
async fn test_volume_creation_and_persistence(docker: Docker) {
    ...
    let claude_data_result = container_utils::exec_command_in_container(
        ...
        vec![
            "cat".to_string(),
            "/root/.claude/test_auth.json".to_string(),
        ],
    )
    .await;
...
async fn test_use_persistant_volume_setting(docker: Docker) {
    ...
    assert!(
        symlink_target.contains("/volume_data/claude"),
        "Symlink should point to volume_data when persistent volume is enabled, but points to: {}",
        symlink_target
    );
