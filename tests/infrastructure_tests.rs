...
async fn test_volume_creation_and_persistence(docker: Docker) {
    ...
    let test_commands = vec![
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "echo '{{\"test\": \"claude_auth_data\"}}' > {}/.claude/test_auth.json",
                container_utils::HOME_DIR
            ),
        ],
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "echo '{{\"api_key\": \"test_claude_config\"}}' > {}/.claude.json",
                container_utils::HOME_DIR
            ),
        ],
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "echo 'test_github_token' > {}/.config/gh/token",
                container_utils::HOME_DIR
            ),
        ],
    ];
    ...
    container_utils::clear_coding_session(&docker, &container_name)
        .await
    ...
    let claude_data_result = container_utils::exec_command_in_container(
        ...
        vec![
            "cat".to_string(),
            format!("{}/.claude/test_auth.json", container_utils::HOME_DIR),
        ],
    )
    .await;

    assert!(
        claude_data_result.is_ok(),
        "Should be able to read Claude auth data"
    );
    let claude_data = claude_data_result.unwrap();
    assert!(
        claude_data.contains("claude_auth_data"),
        "Claude auth data should persist"
    );
    ...
    let claude_config_result = container_utils::exec_command_in_container(
        ...
        vec![
            "cat".to_string(),
            format!("{}/.claude.json", container_utils::HOME_DIR),
        ],
    )
    .await;

    assert!(
        claude_config_result.is_ok(),
        "Should be able to read Claude config file"
    );
    let claude_config = claude_config_result.unwrap();
    assert!(
        claude_config.contains("test_claude_config"),
        "Claude config file should persist"
    );
    ...
    let github_data_result = container_utils::exec_command_in_container(
        ...
        vec![
            "cat".to_string(),
            format!("{}/.config/gh/token", container_utils::HOME_DIR),
        ],
    )
    .await;

    assert!(
        github_data_result.is_ok(),
        "Should be able to read GitHub auth data"
    );
    let github_data = github_data_result.unwrap();
    assert!(
        github_data.contains("test_github_token"),
        "GitHub auth data should persist"
    );
...
async fn test_volume_isolation_between_users(docker: Docker) {
    ...
    let user1_result = container_utils::exec_command_in_container(
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "echo 'user1_secret' > {}/.claude/user_data.txt",
                container_utils::HOME_DIR
            ),
        ],
    )
    ...
    let user2_result = container_utils::exec_command_in_container(
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "echo 'user2_secret' > {}/.claude/user_data.txt",
                container_utils::HOME_DIR
            ),
        ],
    )
    ...
    let user1_data = container_utils::exec_command_in_container(
        ...
        vec![
            "cat".to_string(),
            format!("{}/.claude/user_data.txt", container_utils::HOME_DIR),
        ],
    )
    .await
    .unwrap();
    ...
    assert!(
        user1_data.contains("user1_secret"),
        "User 1 should see only their data"
    );
    assert!(
        user2_data.contains("user2_secret"),
        "User 2 should see only their data"
    );
    assert!(
        !user1_data.contains("user2_secret"),
        "User 1 should not see user 2's data"
    );
    assert!(
        !user2_data.contains("user1_secret"),
...
async fn test_volume_name_generation() {
    ...
    assert_ne!(
        ...
        "Different keys should have different volume names"
    );
}
...
async fn test_use_persistant_volume_setting(docker: Docker) {
    ...
    // Verify volume initialization occurred (symlinks should point to data_volume when persistent volume is enabled)
    let symlink_target_result = container_utils::exec_command_in_container(
        &docker,
        client_with_volume.container_id(),
        vec![
            "readlink".to_string(),
            format!("{}/.claude", container_utils::HOME_DIR),
        ],
    )
    .await;

    assert!(
        symlink_target_result.is_ok(),
        "Should be able to read symlink when persistent volume is enabled"
    );
    let symlink_target = symlink_target_result.unwrap();
    assert!(
        symlink_target.contains("/data_volume/claude"),
        "Symlink should point to data_volume when persistent volume is enabled, but points to: {}",
        symlink_target
    );
    ...
    // Verify volume initialization did NOT occur (symlinks should not point to data_volume)
    ...
    // Either readlink should fail (not a symlink) OR it should not point to /data_volume
    if let Ok(symlink_target) = no_symlink_target_result {
        assert!(
            !symlink_target.contains("/data_volume"),
            "Symlink should NOT point to data_volume when persistent volume is disabled, but \
             points to: {}",
            symlink_target
        );
    }
...
fn test_default_timeout_configuration() {
    ...
    println!(
        ...
        default_config.exec_timeout_secs
    );
}
...
fn test_custom_timeout_configuration() {
    ...
    println!(
        "✅ Custom timeout configuration: {} seconds",
        custom_config.exec_timeout_secs
    );
}
...
async fn test_output_buffering_with_timeout() {
    ...
    // Validate results
    assert!(
        !processed_outputs.is_empty(),
        "Should have processed some output"
    );
...
async fn test_buffering_prevents_rapid_transitions() {
    ...
    println!(
        "  📊 Without buffering: {} state transitions",
        state_transitions_without_buffering
    );
    println!(
        "  📊 With buffering: {} state transitions",
        state_transitions_with_buffering
    );
